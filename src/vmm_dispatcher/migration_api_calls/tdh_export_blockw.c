...
api_error_type tdh_export_blockw(gpa_list_info_t gpa_list_info, uint64_t target_tdr_pa)
{
    ...
    for (entry_num = gpa_list_info.first_entry; entry_num <= gpa_list_info.last_entry; entry_num++)
    {
        ...
        do
        {
            ...
            if (return_val != TDX_SUCCESS)
            {
                TDX_ERROR("walk_private_gpa failed - return_val = 0x%llx\n", return_val);
            ...
            {
                TDX_ERROR("sept_lock_acquire_host failed\n");
            ...
            //   - Save W in TDWR
            //   - Update the state encoding bits
            //   - Do not unlock the SEPT entry; this is done later, after TDCS.BW_EPOCH is updated
